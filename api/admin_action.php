<?php
// api/admin_action.php — MySQL Admin Action Handler (Full CRUD Engine)
session_start();
...
switch ($action) {
    ...
    case 'edit_resource':
        ...
        if (!$id || !$title) { echo json_encode(['error' => 'Invalid parameters']); exit; }

        $stmt = $pdo->prepare("UPDATE resources SET title=?, branch=?, sem=?, type=?, by_author=?, file_size=?, download_url=? WHERE id=?");
        $stmt->execute([$title, $branch, $sem, $type, $by, $size, $download_url, $id]);
    ...
    case 'add_course':
        ...
        if (!$title) { echo json_encode(['error' => 'Course title is required']); exit; }
    ...
    case 'edit_course':
        ...
        if (!$id || !$title) { echo json_encode(['error' => 'Invalid course parameters']); exit; }
    ...
    case 'add_job':
        ...
        if (!$title || !$company) { echo json_encode(['error' => 'Title and company are required']); exit; }
    ...
    case 'edit_job':
        ...
        if (!$id || !$title || !$company) { echo json_encode(['error' => 'Invalid internship parameters']); exit; }
    ...
    case 'add_article':
        ...
        if (!$title) { echo json_encode(['error' => 'Article title is required']); exit; }
        ...
        echo json_encode(['success' => true, 'message' => 'Article published successfully', 'id' => $newId]);
        break;
    ...
    case 'edit_article':
        ...
        if (!$id || !$title) { echo json_encode(['error' => 'Invalid article parameters']); exit; }
